...
public class MovieAtlasSearchServiceImpl implements MovieAtlasSearchService {
    ...
    public Collection<Document> moviesByKeywords(String keywords, int limit) {
        ...
        Bson searchStage = search(text(fieldPath("title"), keywords), searchOptions().index(index));
        Bson projectStage = project(fields(excludeId(), include("title", "year","fullplot", "imdb.rating", "poster", "imdb.id")));
